...
void main() {
  group('AppAuthBloc - Sign Up', () {
    test('emits authenticated state on successful sign up', () async {
      ...
      bloc.add(AppAuthEmailChanged('newuser@example.com'));
      await Future.delayed(Duration(milliseconds: 50));
    ...
    test('emits error state on failed sign up', () async {
      final repo = FakeAuthRepository();
      repo.setRegisterShouldFail(true);
      final bloc = AppAuthBloc(
        repository: repo,
        signInUseCase: SignInUseCase(repo),
        signUpUseCase: SignUpUseCase(repo),
